using AutoMapper;
using AutoresPruebas.DTOs;
using AutoresPruebas.Servicios;
...
public class LlavesAPIController : CustomBaseController
{
    ...
    public async Task<ActionResult> CrearLlave(CrearLlaveDTO crearLlaveDTO)
    {
        ...
        if (crearLlaveDTO.TipoLlave == Models.TipoLlave.Gratuita)
        {
            var elUsuarioYaTieneUnaLlaveGratuita = await context.LlavesAPI
                .AnyAsync(x => x.UsuarioId == usuarioId && x.TipoLlave == Models.TipoLlave.Gratuita);
